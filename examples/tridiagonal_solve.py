...
if cp:
    ...
    def tridiagonal_solve_cu(inf: Field3D, diag: Field3D, sup: Field3D, rhs: Field3D, out: Field3D):
        with computation(FORWARD):
            with interval(0, 1):
                sup = sup / diag
                rhs = rhs / diag
...
def main():
    ...
    if cp:
        inf_fld_cu = gt4py.storage.from_array(
            data=inf_data, backend=gpu_backend, default_origin=(nhalo, nhalo, 0), shape=shape, managed_memory=managed_mem
        )
    ...
    tridiagonal_solve_py(inf_fld_py, diag_fld_py, sup_fld_py, rhs_fld_py, out_fld_py, domain=domain)
    ...
    t_start = time.perf_counter()
    tridiagonal_solve_mc(inf_fld_mc, diag_fld_mc, sup_fld_mc, rhs_fld_mc, out_fld_mc, domain=domain)
    t_run = (time.perf_counter() - t_start) * 1e3
    ...
    if cp:
        t_start = time.perf_counter()
        tridiagonal_solve_cu(in_fld_cu, diag_fld_cu, sup_fld_cu, rhs_fld_cu, out_fld_cu, domain=domain)
        t_run = (time.perf_counter() - t_start) * 1e3
        print("%s time: %.6lf" % (gpu_backend, t_run))
        out_fld_cuda.synchronize()
        np.savetxt("tridiag_solve_gpu_output.csv", np.asarray(out_fld_cuda).flatten(), delimiter=',')
